...
netcup_write_envfile() {
  write_file "${NETCUP_ENVFILE}" "0600" "$(cat <<EOF
...
caddy_has_netcup_module() { command -v /usr/local/bin/caddy >/dev/null 2>&1 || return 1; /usr/local/bin/caddy list-modules 2>/dev/null | grep -q '^dns.providers.netcup$'; }
...
caddy_install_systemd_unit() {
  write_file /etc/systemd/system/caddy.service "0644" "$(cat <<'EOF'
[Unit]
...
caddy_write_caddyfile() {
  ...
  if [[ "${CADDY_CERT_MODE}" == "dns01_wildcard" ]]; then
    write_file /etc/caddy/Caddyfile "0644" "$(cat <<EOF
${global_email_block}
...
$( if [[ "${DASH_ENABLE:-false}" == "true" && "${DASH_BASICAUTH:-false}" == "true" ]]; then cat <<EOR
  @kube host ${DASH_HOST}
  handle @kube {
    basicauth {
